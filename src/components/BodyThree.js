    ...
    <div className="p-6 mt-6 bg-white shadow-lg rounded-lg">
      ...
      <section className="mb-8">
        ...
        <p className="text-lg text-gray-600 mb-4">
          As per the latest report, the egg rate in {location} has reached ₹{todayRate} per piece. But this is not the highest price for eggs in the city in the last one year. The price of eggs has been on the rise in the city for the last few months. The rise in the price of eggs is due to the increase in the cost of chicken feed. The cost of chicken feed has gone up by 10% in the last few months. This has led to an increase in the price of eggs in {location}.
        </p>
        <p className="text-lg text-gray-600 mb-4">
          Egg prices in {location} have been on the rise in recent months, due to a variety of factors. The cost of feed, transportation, and labor have all increased, leading to higher prices at the farm gate. Consumers are now paying more for eggs, with the average price of a tray of eggs now exceeding ₹{(todayRate * 30).toFixed(2)}. This is a significant increase from just a few years ago, when a tray of eggs could be purchased for as little as ₹{(todayRate * 30 - 20).toFixed(2)}. The higher prices are having an impact on egg consumption in {location}, as many families are cutting back on their consumption of this staple food. While the current situation is difficult for consumers, it is important to remember that egg prices are still relatively low compared to other staples such as rice and wheat.
        </p>
        <p className="text-lg text-gray-600 mb-4">
      ...
      <section className="mb-8">
        ...
        <p className="text-lg text-gray-600 mb-4">
          Today’s date is {new Date().toDateString()} and we can see that the egg rate in {location} today is ₹{todayRate}. But if we compare today’s date with yesterday’s date, we can see that yesterday the price of eggs in {location} was ₹{todayRate}.
        </p>
        <table className="min-w-full bg-white divide-y divide-gray-200 mb-4">
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            <tr>
              ...
              <td className="px-6 py-4 whitespace-nowrap">₹{todayRate}</td>
            </tr>
            <tr>
              <td className="px-6 py-4 whitespace-nowrap">Egg rate 7 days ago</td>
              <td className="px-6 py-4 whitespace-nowrap">₹{rate7DaysAgo}</td>
            </tr>
            <tr>
              <td className="px-6 py-4 whitespace-nowrap">Weekly Change in Egg rate (₹)</td>
              <td className="px-6 py-4 whitespace-nowrap">{weeklyChange}</td>
            </tr>
            <tr>
              <td className="px-6 py-4 whitespace-nowrap">% Weekly Change in Egg Rate</td>
              <td className="px-6 py-4 whitespace-nowrap">{weeklyChangePercent}%</td>
            </tr>
            <tr>
            ...
            <tr>
              ...
              <td className="px-6 py-4 whitespace-nowrap">₹{(todayRate + 0.35).toFixed(2)}</td>
            </tr>
            <tr>
              <td className="px-6 py-4 whitespace-nowrap">Supermarket Price (₹)</td>
              <td className="px-6 py-4 whitespace-nowrap">₹{(todayRate + 0.45).toFixed(2)}</td>
            </tr>
            <tr>
              <td className="px-6 py-4 whitespace-nowrap">Average Price till today</td>
              <td className="px-6 py-4 whitespace-nowrap">₹{averagePrice}</td>
            </tr>
          </tbody>
        </table>
      </section>
      ...
      <section className="mb-8">
        ...
        <p className="text-lg text-gray-600 mb-4">
          If you are looking to buy eggs in bulk then you can consider buying an egg tray. The egg tray which usually holds 30 eggs in a tray costs ₹{(todayRate * 30).toFixed(2)}. Looking at the graph above you can see how the egg rate has fluctuated on a daily basis. In the above graph you can also see the weekly change of egg prices. There has been a change in the price of eggs by {weeklyChange} compared to last week. The weekly percentage change in the price of eggs is {weeklyChangePercent}%.
        </p>
      </section>
    </div>
  );
