...
// Get all cities from database - try the normalized tables first
try {
    $cities = [];
    
    // Try to get cities from normalized tables
    $sql = "SELECT c.name AS city, s.name AS state FROM cities c JOIN states s ON c.state_id = s.id ORDER BY c.name";
    ...
    if ($result && $result->num_rows > 0) {
        ...
        // Fall back to original table if no results from normalized tables
    ...
    // Fall back to original table if there's an error with normalized tables
    $sql = "SELECT DISTINCT city, state FROM egg_rates ORDER BY city";
    $result = $conn->query($sql);
