<?php
/**
 * API: Remove State or City
 * 
 * Removes a state or city from the database
 */
...
try {
    ...
    $success = $locationModel->removeLocation($type, $name, $state);
    
    if (!$success) {
        ApiResponse::error("Failed to remove $type: $name");
    }
    ...
    ApiResponse::success(null, "$type removed successfully");
} catch (Exception $e) {
    ApiResponse::error('Failed to remove location: ' . $e->getMessage());
}
